...
def create_lookup_tables(text):
    ...
    counts = Counter(w for line in text for w in line.split())
    vocab = map(lambda x:x[0],sorted(counts.items(),key=lambda x:-x[1])) 
    vocab_to_int = dict((word, idx+start_idx) for idx,word in enumerate(vocab))
    vocab_to_int['<empty>'] = empty
    vocab_to_int['<eos>'] = eos
    
    int_to_vocab = dict((idx,word) for word,idx in vocab_to_int.items())
